...
const DEFAULT = {
  storeName: 'OrientPerfumes', storeEmail: 'info@orientperfumes.com',
  storePhone: '+57 300 000 0000', storeAddress: 'Rionegro, Antioquia, Colombia',
  storeDesc: 'Fragancias exclusivas de lujo. Tu destino para los mejores perfumes del mundo.',
  stockThreshold: 20, currency: 'COP',
...
export default function Ajustes() {
  ...
  return (
    <div style={{ padding: 32, color: '#E8DCC8', maxWidth: 640 }}>
      ...
      <Card titulo="Notificaciones">
        {[
          ['notifStock',  'Alertas de Stock Bajo',  'Notificar cuando el stock sea menor a ' + settings.stockThreshold + ' unidades'],
          ...
          <div key={key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid rgba(201,168,76,0.05)' }}>
            ...
            <label style={{ position: 'relative', display: 'inline-block', width: 40, height: 22, flexShrink: 0 }}>
              <input type="checkbox" checked={settings[key]} onChange={e => set(key, e.target.checked)} style={{ opacity: 0, width: 0, height: 0 }} />
              <span style={{ position: 'absolute', cursor: 'pointer', inset: 0, background: settings[key] ? '#C9A84C' : '#333', borderRadius: 22, transition: '0.3s' }}>
                <span style={{ position: 'absolute', content: '', height: 16, width: 16, left: settings[key] ? 20 : 3, bottom: 3, background: 'white', borderRadius: '50%', transition: '0.3s' }} />
              </span>
            </label>
          </div>
        ))}
      ...
      <Card titulo="Inventario">
        ...
        <Label>Moneda</Label>
        <select style={{ ...inp, maxWidth: 200 }} value={settings.currency} onChange={e => set('currency', e.target.value)}>
      ...
      <button onClick={guardar} style={{ background: '#C9A84C', border: 'none', borderRadius: 6, padding: '10px 24px', color: '#0a0a08', cursor: 'pointer', fontFamily: 'Cinzel, serif', fontSize: 11, letterSpacing: '0.1em', marginTop: 8 }}>
        Guardar Cambios
      </button>
    </div>
  );
